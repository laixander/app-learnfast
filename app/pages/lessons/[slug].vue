    ...
    <div class="relative z-10 flex flex-col gap-8 max-w-4xl mx-auto">
      ...
      <header
        ...
        <div class="flex items-center gap-4">
          <UButton to="/lessons" icon="i-ph-arrow-left-bold" variant="ghost" color="neutral" class="rounded-2xl" />
          <div class="flex flex-col">
            <h1 class="text-2xl font-black text-toned leading-none">{{ lesson.title }}</h1>
            <span class="text-sm font-bold text-primary-600 uppercase tracking-widest mt-1">{{ lesson.category }}</span>
          </div>
        </div>
        ...
        <Transition name="fade-slide" mode="out-in">
          ...
          <div v-if="!isQuizMode && !isFinished" :key="`step-${currentStepIndex}`"
            ...
            <div class="flex items-center gap-4 mt-6">
              <UButton v-if="currentStepIndex > 0" label="Previous" variant="subtle" color="neutral" size="xl"
                class="font-black px-10 rounded-2xl" @click="prevStep" />
              <UButton :label="currentStepIndex === lesson.steps.length - 1 ? 'Start Quiz!' : 'Next Step'"
                color="primary" size="xl" class="font-black px-12 py-4 rounded-2xl shadow-xl hover:scale-105"
                @click="nextStep" />
            </div>
          </div>
          ...
          <div v-else-if="isQuizMode && !isFinished" key="quiz"
            ...
            <UButton label="Submit My Answers!" block size="xl" color="primary"
              class="font-black py-6 rounded-[2rem] shadow-2xl mt-4" :disabled="quizAnswers.length < lesson.quiz.length"
              @click="submitQuiz" />
          </div>
          ...
          <div v-else-if="isFinished" key="finished"
            ...
            <div class="relative z-10 flex flex-col items-center gap-6">
              ...
              <p class="text-2xl opacity-90 font-medium">You completed the <span class="underline underline-offset-8">{{
                  lesson.title }}</span> lesson!</p>

              <div class="flex gap-4 mt-4">
                <div class="bg-white/20 backdrop-blur-md px-10 py-6 rounded-3xl border-2 border-white/30 flex flex-col">
                ...
                <div class="bg-yellow-400 text-yellow-900 px-10 py-6 rounded-3xl border-2 border-white flex flex-col">
