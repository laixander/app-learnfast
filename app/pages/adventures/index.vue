    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      <UCard v-for="adv in filteredAdventures" :key="adv.title"
        ...
        <div class="flex items-start justify-between">
          ...
          <div class="bg-white/80 backdrop-blur-sm px-6 py-2 rounded-full font-black text-primary-600 shadow-md">
            {{ adv.progress }}%
          </div>
        </div>
        <div>
        ...
        <div class="flex flex-col gap-4 mt-2">
          ...
          <UButton :to="`/adventures/${adv.slug}`" :label="adv.progress > 0 ? 'Continue' : 'Start Adventure'" block
            size="xl" class="font-black rounded-2xl py-4 hover:translate-x-2 transition-transform"
