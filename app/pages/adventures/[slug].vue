...
<template>
  <div v-if="adventure" class="relative">
    ...
    <div class="relative z-10 flex flex-col gap-10">
      <!-- Back Button -->
      <UButton to="/adventures" icon="i-ph-arrow-left-bold" label="Back to Adventures" variant="ghost" color="neutral"
        class="self-start font-black text-lg hover:bg-primary-100 hover:text-primary-600 rounded-2xl px-6" />

      <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
        <!-- Main Content -->
        <div class="lg:col-span-2 flex flex-col gap-8">
          <div class="bg-white/40 backdrop-blur-xl p-8 md:p-12 rounded-[3.5rem] border-4 border-white shadow-2xl">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-10">
              <div class="bg-white/60 p-6 rounded-xl border-2 border-white flex flex-col items-center gap-2 shadow-lg">
          ...
          <div
            ...
            <div class="relative z-10 flex flex-col gap-8 items-center text-center max-w-3xl mx-auto">
              ...
              <div
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div v-for="(goal, index) in adventure.startContent?.goals" :key="index"
                    ...
                    <div class="size-12 rounded-2xl bg-white/20 flex items-center justify-center shrink-0 shadow-lg">
                      <UIcon :name="goal.icon" :class="['size-7', goal.color]" />
                    </div>
                    <div>
                      <p class="font-black text-xl">{{ goal.title }}</p>
                      <p class="text-white/70 text-sm font-medium leading-snug">{{ goal.description }}</p>
                    </div>
                  </div>
                  ...
                  <template v-if="!adventure.startContent?.goals">
                    <div class="flex items-start gap-4">
                      <div class="size-10 rounded-2xl bg-white/20 flex items-center justify-center shrink-0">
                        <UIcon name="i-ph-book-open-duotone" class="size-6 text-yellow-300" />
                      </div>
                      <div>
                        <p class="font-black text-lg">Master Lessons</p>
                        <p class="text-white/70 text-sm font-medium leading-snug">Complete all {{ adventure.lessons }}
              ...
              <div class="flex flex-col gap-4 items-center mt-4">
                <UButton :to="firstLesson ? `/lessons/${firstLesson.slug}` : '/lessons'" label="Start Adventure! →"
                  size="xl" color="neutral"
                ...
                <p class="text-white/60 text-sm font-bold uppercase tracking-widest flex items-center gap-2">
        ...
        <div class="flex flex-col gap-8">
            ...
            <div class="flex flex-col gap-4">
              ...
              <p class="text-sm font-medium text-muted mt-2">
                {{
                  adventure.progress >
                    0 ?
                    'You are doing great! Keep going to earn your badge.' : 'Start today to begin your journey!'
                }}
              </p>
            </div>
          </div>
          ...
          <div
            ...
            <ul class="flex flex-col gap-4">
              <li v-for="(goal, index) in adventure.startContent?.goals" :key="index" class="flex gap-3 items-start">
                <div class="size-6 rounded-full bg-emerald-100 flex items-center justify-center mt-1 shrink-0">
              ...
              <!-- Fallback if no goals defined -->
              <li v-if="!adventure.startContent?.goals" v-for="i in 3" :key="i" class="flex gap-3 items-start">
                <div class="size-6 rounded-full bg-emerald-100 flex items-center justify-center mt-1 shrink-0">
                  <UIcon name="i-ph-check-bold" class="text-emerald-600 size-4" />
                </div>
                <span class="text-lg font-medium text-toned leading-snug">Amazing goal #{{ i }} that you will
                  master!</span>
              </li>
            </ul>
          </div>
